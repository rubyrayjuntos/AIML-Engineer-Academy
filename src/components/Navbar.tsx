...
import { Sparkles, Award, Search, BookOpen, Lock } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const progressPercent = Math.round((completedCount / moduleTotal) * 100);
  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-4 md:px-8 py-3.5 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {certificateReady ? <Award className="w-4 h-4 text-amber-700" /> : <Lock className="w-4 h-4 text-slate-500" />}
