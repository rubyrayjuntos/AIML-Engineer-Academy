...
| Browser IPI (page-as-instruction) | Dual-LLM firewall: Minimizer on tool inputs + quarantined Sanitizer → `SafeObservation` for privileged planner; `evaluate_js` rejected; consequential clicks require HITL | Optional live sanitizer (`ACADEMY_DUAL_LLM=1`) still has no tools/credentials; Playwright must keep the same quarantine boundary |
