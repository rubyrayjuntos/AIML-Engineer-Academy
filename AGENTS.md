...
- Module 3 lab is dual-lane: Customer Success HITL **and** read-only SQL/MCP (`SQLQueryResult` seam, `sql_guard`, analytics SQLite, MCP `get_table_schema` / `execute_readonly_sql`) plus an offline DSPy compile stub. Optional live structured output: leave `ACADEMY_LIVE_LLM` unset in Cloud Agent/CI; plan via `python -m app.pydantic_ai_optional` (`claims.pydantic_ai_executed=false`). Default pytest expects **29 passed, 1 skipped** (`live_llm`). Hub includes `SqlAgentSimulator` (teaching-only).
